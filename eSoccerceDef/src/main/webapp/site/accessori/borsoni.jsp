...
<input type="text" placeholder="Ricerca il tuo prodotto..." name="search"></form>
	...
	<%
	Collection<?> borsoni = (Collection<?>) request.getAttribute("borsoni"); 
	String error = (String) request.getAttribute("error");
	if(borsoni == null && error == null){ 
...
<%
		if(borsoni != null && borsoni.size() > 0)
		{
			Iterator<?> it = borsoni.iterator();
			while(it.hasNext())
			{
...
<% } %>
<tr><td style="vertical-align:center;"><form action="<%= response.encodeURL("./RecensioniControl") %>" method="POST" id="recensione"><input type="hidden" id="nomeprodotto" name="nomeprodotto" value="<%=bean.getNome()%>"><input type="hidden" id="emailrecensione" name="emailrecensione" value="<%=user.getEmail() %>"><button type="submit">Aggiungi una recensione!</button> <br>
